...
export default function GrinderList() {
  const { grinders, isLoading, error } = useGetGrinder();
  ...
  const [type, setType] = useState<"all" | "manual" | "automatic">("all");

  const { searchTerm, setSearchTerm } = useSearch();

  const filteredData = grinders
    .filter((item) =>
      ...
      <div className="w-full flex justify-between">
        <div className="flex gap-4">
          <Tabs
            value={type}
            onValueChange={(value) =>
              setType(value as "all" | "manual" | "automatic")
            }
          >
            <TabsList>
